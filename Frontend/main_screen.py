...
class MainScreen(tk.Frame):
    def __init__(self, controller):
        ...
        for i in range(7):  # 7 columns (1 for "Add" button, 3 for OptionMenus, 1 for empty space, and 2 for search area)
        ...
        status_options = ["All status", *STATUS]  # Cập nhật "All" thành "All status"
        self.status_var = tk.StringVar(nav_frame)
        self.status_var.set("All status")  # Giá trị mặc định là "All status"
        ...
        task_options = ["All categories", *CATEGORIES]  # Cập nhật "All" thành "All category"
        self.task_var = tk.StringVar(nav_frame)
        self.task_var.set("All categories")  # Giá trị mặc định là "All categories"
        ...
        priority_options = ["All priorities", *PRIORITY]  # Cập nhật "All" thành "All priorities"
        self.priority_var = tk.StringVar(nav_frame)
        self.priority_var.set("All priorities")  # Giá trị mặc định là "All priorities"
    ...
    def on_select_option_menu(self, attribute, value):
        ...
        if value == "All status":  # Kiểm tra xem người dùng có chọn "All status" không
            self.filter["status"] = "All"  # Lọc tất cả các trạng thái
        elif value == "All categories":  # Kiểm tra xem người dùng có chọn "All categories" không
            self.filter["category"] = "All"  # Lọc tất cả các category
        elif value == "All priorities":  # Kiểm tra xem người dùng có chọn "All priorities" không
            self.filter["priority"] = "All"  # Lọc tất cả các priority
        self.update_screen()
