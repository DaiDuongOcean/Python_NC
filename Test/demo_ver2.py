...
def attach_image():
    ...
    if image_path:
        ...
        messagebox.showinfo("Thông báo", "Không có hình ảnh được chọn.")
...
def create_note():
    ...
    reminder_time = reminder_entry.get().strip()
    ...
    try:
        ...
        messagebox.showerror("Lỗi", "Định dạng thời gian nhắc nhở không hợp lệ (YYYY-MM-DD HH:MM).")
        return
...
def set_reminder(title, content, image_path, reminder_time):
    try:
        ...
        print(f"Lỗi khi thiết lập lời nhắc: {e}")
...
def on_note_select(event):
    ...
    if selected_item:
        ...
        reminder_entry.delete(0, tk.END)
        reminder_entry.insert(0, note.get("reminder_time", ""))
...
content_frame.grid(row=1, column=1, columnspan=3, padx=10, pady=5)
content_text = tk.Text(content_frame, height=10, width=50, wrap=tk.WORD)
content_scroll = tk.Scrollbar(content_frame, command=content_text.yview)
...
notes_scroll = tk.Scrollbar(app, command=notes_list.yview)
notes_list.configure(yscrollcommand=notes_scroll.set)
notes_list.grid(row=6, column=0, columnspan=4, padx=10, pady=10, sticky="nsew")
notes_scroll.grid(row=6, column=4, sticky="ns")
